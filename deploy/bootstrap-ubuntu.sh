...
BROWSER_TASK_CONCURRENCY="${BROWSER_TASK_CONCURRENCY:-5}"
BROWSER_MAINTENANCE_CONCURRENCY="${BROWSER_MAINTENANCE_CONCURRENCY:-2}"
CDK_TEAM_INVITE_TIMEOUT_MS="${CDK_TEAM_INVITE_TIMEOUT_MS:-300000}"
OVERFLOW_REBALANCE_WORKSPACE_CONCURRENCY="${OVERFLOW_REBALANCE_WORKSPACE_CONCURRENCY:-2}"
MEMBER_REMOVAL_WORKSPACE_CONCURRENCY="${MEMBER_REMOVAL_WORKSPACE_CONCURRENCY:-3}"
MEMBER_REMOVAL_MEMBER_CONCURRENCY="${MEMBER_REMOVAL_MEMBER_CONCURRENCY:-6}"
MEMBER_CLEANUP_SYNC_CONCURRENCY="${MEMBER_CLEANUP_SYNC_CONCURRENCY:-3}"

if [[ -z "$REPO_URL" ]]; then
  echo "Usage: bash bootstrap-ubuntu.sh <repo-url> [app-dir]"
  echo "Optional env: APP_BRANCH, CERTBOT_DOMAINS, CERTBOT_EMAIL, PUBLIC_BASE_URL, CDK_TEAM_WORKER_CONCURRENCY, BROWSER_TASK_CONCURRENCY, BROWSER_MAINTENANCE_CONCURRENCY, CDK_TEAM_INVITE_TIMEOUT_MS, OVERFLOW_REBALANCE_WORKSPACE_CONCURRENCY, MEMBER_REMOVAL_WORKSPACE_CONCURRENCY, MEMBER_REMOVAL_MEMBER_CONCURRENCY, MEMBER_CLEANUP_SYNC_CONCURRENCY, ADMIN_BASIC_AUTH_USER, ADMIN_BASIC_AUTH_PASS"
  exit 1
fi
...
mkdir -p /etc/systemd/system/openai-monitor.service.d
{
  ...
  echo "Environment=BROWSER_MAINTENANCE_CONCURRENCY=$BROWSER_MAINTENANCE_CONCURRENCY"
  echo "Environment=CDK_TEAM_INVITE_TIMEOUT_MS=$CDK_TEAM_INVITE_TIMEOUT_MS"
  echo "Environment=OVERFLOW_REBALANCE_WORKSPACE_CONCURRENCY=$OVERFLOW_REBALANCE_WORKSPACE_CONCURRENCY"
  echo "Environment=MEMBER_REMOVAL_WORKSPACE_CONCURRENCY=$MEMBER_REMOVAL_WORKSPACE_CONCURRENCY"
  echo "Environment=MEMBER_REMOVAL_MEMBER_CONCURRENCY=$MEMBER_REMOVAL_MEMBER_CONCURRENCY"
  echo "Environment=MEMBER_CLEANUP_SYNC_CONCURRENCY=$MEMBER_CLEANUP_SYNC_CONCURRENCY"
